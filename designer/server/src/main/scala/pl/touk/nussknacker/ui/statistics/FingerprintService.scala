...
class FingerprintService(dbioRunner: DBIOActionRunner, fingerprintRepository: FingerprintRepository[DB])(
  ...
  private lazy val nuFingerprintFileName = "nussknacker.fingerprint"

  private lazy val fingerprintFile = new File(
    Try(Option(System.getProperty("java.io.tmpdir"))).toOption.flatten.getOrElse("/tmp"),
    nuFingerprintFileName
  )

  def fingerprint(config: UsageStatisticsReportsConfig): Future[Fingerprint] = {
    ...
    config.fingerprint.filterNot(_.isBlank) match {
      ...
      case None                        => fetchOrGenerate()
    }
  }

  private def fetchOrGenerate(): Future[Fingerprint] =
    for {
      ...
      result <- dbFingerprint match {
        ...
        case None => {
          val generated = readFingerprintFromFile().getOrElse(randomFingerprint)
  ...
  private def readFingerprintFromFile(): Option[String] =
    Try(FileUtils.readFileToString(fingerprintFile, StandardCharsets.UTF_8)) match {
      case Failure(_)     => None
